...
for conversation in conversations[:-1]:
    _comversation = conversation.split(' +++$+++ ')[-1][1:-1].replace("'","").replace(" ", "")
    conversations_ids.append(_comversation.split(','))

#getting separetely the questions and answers
questions = []
answers = []
...
#creating dictionary that maps each word to its number of occurances
...
#adding the last tokens these two dictionaries
...
#and the replacing all the words that were filtered out by <OUT>
...
#sorting questions ans answers by the lengh of the questions
...
def decode_training_set(encoder_state, decoder_cell, decoder_embedded_input, sequence_length, decoding_scope, output_function,keep_prob, batch_size):
    ...
    training_decoder_function = tf.contrib.seq2seq.attention_decoder_fn_train(encoder_state[0],
                                                                              ...
                                                                              name = "attn_dec_train")
    decoder_output, decoder_final_state, decoded_final_context_state = tf.contrib.seq2seq.dynamic_rnn_decoder(decoder_cell,
...
def decoder_rnn(decoder_embedded_input, decoder_embeddings_matrix, encoder_state, num_words, sequence_lenght, rnn_size, num_layers, word2int, keep_prob, batch_size):
...
def seq2seq_model(inputs, targets, keep_prob, batch_size, sequence_length, answers_num_words, questions_num_words, encoder_embedding_size, decoder_embedding_size, rnn_size, num_layers,questionswords2int):
    ...
    encoder_state = encoder_rnn(encoder_embedding_input,
                                rnn_size,
                                num_layers,
                                keep_prob,
                                sequence_length)
...
#splitting data into batches of answers and questions
    ...
    if batch_index % batch_index_check_validation_loss == 0 and batch_index > 0:
        ...
        for batch_index_validation, (padded_questions_in_batch, padded_answers_in_batch) in enumerate(split_into_batches(validation_questions, validation_answers, batch_size)):
            ...
            if average_validation_loss_error <= min(list_validation_loss_error):
                ...
                print("sorry i do not speak better, i need the practice more")
                early_stopping_check += 1
                if early_stopping_check == early_stopping_stop:
                    break
    if early_stopping_check == early_stopping_stop:
        print("I can not speak better anymore:This is the best")
print("Game Over")
